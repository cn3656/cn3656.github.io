  ...
  _loadDirectory(dir, layout) {
    ...
    for (const file of files) {
      try {
        ...
        console.warn(`[Atom] Failed to parse ${file}:`, e.message);
      }
    }
  }
    ...
    for (const entry of entries) {
      ...
      if (entry.endsWith('.md') && fs.statSync(fullPath).isFile()) {
        try {
          ...
          console.warn(`[Atom] Failed to parse page ${entry}:`, e.message);
        }
        ...
        if (fs.existsSync(indexPath)) {
          try {
            ...
            console.warn(`[Atom] Failed to parse ${indexPath}:`, e.message);
          }
        }
      }
    }
  ...
  _loadDataFiles(sourceDir) {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        console.warn(`[Atom] Failed to parse data file ${entry}:`, e.message);
      }
    }
  }
