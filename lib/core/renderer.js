...
class Renderer {
  ...
  _setupMarked() {
    ...
    renderer.code = function(codeObj) {
      ...
      const lineNumbers = highlightConfig.line_number
        ? `<div class="line-numbers">${lines.map((_, i) => `<span>${i + 1}</span>`).join('')}</div>`
        : '';
      
      return `<figure class="highlight ${language}">${lineNumbers}<table><tr><td class="code"><pre><code class="${language}">${highlighted}</code></pre></td></tr></table></figure>`;
    };
