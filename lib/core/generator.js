...
class Generator {
  ...
  async init() {
    ...
    if (hexoLoaded > 0) {
      console.log(`[Atom] ${hexoLoaded} hexo plugins loaded via compat layer`);
  ...
  async generate() {
    console.log('[Atom] Starting generation...');
    ...
    console.log(`[Atom] Loaded ${this.data.posts.length} posts, ${this.data.pages.length} pages`);
    ...
    console.log(`[Atom] Generated ${this.generated} files to ${publicDir}`);
  ...
  async _generatePosts(publicDir) {
    for (const post of this.data.posts) {
      ...
      try {
        ...
        console.error(`[Atom] Error rendering post "${post.title}":`, e.message);
      }
    }
  }
      ...
      try {
        const html = await this.theme.render(templateName, {
          page: {
            ...
            comments: page.frontMatter && page.frontMatter.comments,
            date: page.date,
        ...
        console.error(`[Atom] Error rendering page "${page.title}":`, e.message);
      }
    }
  }
  ...
  async _generateIndex(publicDir) {
    ...
    for (let page = 1; page <= total; page++) {
      ...
      try {
        ...
        console.error(`[Atom] Error rendering index page ${page}:`, e.message);
      }
    }
  }
  ...
  async _generateArchives(publicDir) {
    ...
    try {
      ...
      console.error('[Atom] Error rendering archives:', e.message);
    }
  }
  ...
  async _generateCategories(publicDir) {
    ...
    try {
      ...
      console.error('[Atom] Error rendering categories:', e.message);
    }
    ...
    for (const cat of categories) {
      try {
        ...
        console.error(`[Atom] Error rendering category "${cat.name}":`, e.message);
      }
    }
  }
  ...
  async _generateTags(publicDir) {
    ...
    try {
      ...
      console.error('[Atom] Error rendering tags:', e.message);
    }
    ...
    for (const tag of tags) {
      try {
        ...
        console.error(`[Atom] Error rendering tag "${tag.name}":`, e.message);
      }
    }
  }
  
  _generateSitemap(publicDir) {
    const root = this.config.get('url', 'https://atom.github.io');
    const urls = [];
...
${urls.map(u => `  <url>
  ...
  _generateFeed(publicDir) {
    const root = this.config.get('url', 'https://atom.github.io');
    const title = this.config.get('title', 'Atom Blog');
