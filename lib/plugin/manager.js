...
class PluginManager {
  ...
  register(name, plugin) {
    if (this.plugins.has(name)) {
      console.warn(`[Atom] Plugin "${name}" already registered, skipping`);
      return;
    }
    ...
    if (plugin.hooks) {
      for (const [hook, fn] of Object.entries(plugin.hooks)) {
        if (this.hooks[hook]) {
          ...
          console.warn(`[Atom] Unknown hook "${hook}" in plugin "${name}"`);
        }
      }
    }
    ...
    console.log(`[Atom] Plugin registered: ${name}`);
  }
  ...
  async applyHook(hookName, data, context) {
    ...
    for (const { plugin, fn } of handlers) {
      try {
        ...
        console.error(`[Atom] Error in plugin "${plugin}" hook "${hookName}":`, e.message);
      }
    }
  ...
  loadPlugins(configuredPlugins) {
    ...
    for (const pluginRef of pluginsToLoad) {
      try {
        ...
        console.warn(`[Atom] Failed to load plugin "${pluginRef}":`, e.message);
      }
    }
    
    console.log(`[Atom] ${this.plugins.size} plugins loaded`);
  }
