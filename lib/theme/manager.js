...
class ThemeManager {
  ...
  _loadI18n() {
    ...
    // 内置基础翻译
    return {
      home: 'Home', archives: 'Archives', categories: 'Categories',
      tags: 'Tags', about: 'About', tag: 'Tag', category: 'Category',
      prev: 'Prev', next: 'Next', older: 'Older', newer: 'Newer'
    };
  }
  ...
  _buildContext(data = {}) {
    ...
    const partial = (name, locals = {}) => {
      ...
      if (!partialPath) {
        console.warn(`[Ation] partial not found: ${name}`);
        return '';
      }
      try {
        ...
        console.warn(`[Ation] partial error (${name}):`, e.message);
        return '';
      }
    };
    ...
    const context = {
      ...
      site: { posts: hexifyPosts(data.posts || []), categories: data.categories || [], tags: data.tags || [] },
